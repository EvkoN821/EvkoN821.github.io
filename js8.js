$(document).ready(function () {
    ...
    $(document).mouseup(function (e) {
        if (!$(".window").is(e.target) && $(".window").has(e.target).length === 0) formHide();
    });

    $("#check").change(function () { 
        if ($("#check").is(":checked")) $("#submitButton").prop("disabled", false);
    ...
    window.addEventListener("popstate", function (event) { 
        if (event.state.isActive) formShow();
		else formHide();       
    });

    const ajaxSend = (formData) => {
		fetch ("https://formcarry.com/s/EzbPKuFdyzV", {method: "POST", headers: { "Content-Type": "application/json", "Accept": "application/json"}, body: JSON.stringify(formData)})
